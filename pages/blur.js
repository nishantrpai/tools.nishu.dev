...
export default function BlurArea() {
  ...
  const applyBlur = (ctx, img, forDownload = false) => {
    ...
    if (selectedAreaOnly) {
      // First draw the original unblurred image
      ctx.filter = 'none'
      ctx.drawImage(img, 0, 0)

      // Then draw the blurred selected area
      ...
      // If no area is selected, draw the original image
      ctx.filter = 'none'
      ctx.drawImage(img, 0, 0)
    }
  }
  ...
  return (
    <>
      <Head>
        <title>Blur Area</title>
        <meta name="description" content="Blur specific area from image" />
        <link rel="icon" href="/favicon.ico" />
      </Head>
      <main>
        <h1>Blur Area</h1>
        <h2 className={styles.description}>Select area to blur, rest will remain unchanged</h2>
