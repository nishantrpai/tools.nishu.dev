...
export default function ColorFilter() {
  ...
  const [filterColor, setFilterColor] = useState('#54FF56') // Default color (84, 255, 86)
  ...
  }, [image, filterColor, filterThreshold, selectedAreaOnly, selectionRect, reverseFilter])
      ...
      for (let x = 0; x < canvas.width; x++) {
        ...
        if (isInSelectedArea) {
          ...
          if ((reverseFilter && avg <= filterThreshold) || (!reverseFilter && avg > filterThreshold)) {
            data[i] = filterRgb.r // Red channel
            data[i + 1] = filterRgb.g // Green channel
            data[i + 2] = filterRgb.b // Blue channel
  ...
  return (
    <>
      <Head>
        ...
        <meta name="description" content="Color Filter" />
      ...
      <main style={{
        ...
        <h1>Color Filter</h1>
        <h2 className={styles.description}>Add color filter on any image</h2>
        ...
        <div style={{ marginTop: '20px' }}>
          <label htmlFor="filterColor">Filter Color: </label>
          <input
            type="color"
            id="filterColor"
            value={filterColor}
