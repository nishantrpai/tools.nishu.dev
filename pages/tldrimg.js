...
export default function WhichFont() {
  ...
  const getFont = async () => {
    ...
    const response = await fetch('/api/gpt', {
      method: 'POST',
      body: JSON.stringify({
        prompt: `please extract the text from the image below and summarize in one line. Do not skip any large texts.Do not add prefix/suffix before extracted text.\n\nText:`,
        image_url: dataURL,
        model: 'gpt-4o-mini'
      }),
    });
  ...
  return (
    <>
      <Head>
        ...
        <meta name="description" content="TLDR text from image" />
