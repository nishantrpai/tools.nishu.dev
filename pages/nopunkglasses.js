...
export default function HigherHat() {
  ...
  const higherHat = '/nopunkglass.png'
  ...
  return (
    <>
      <Head>
        <title>Nopunk glasses</title>
        <meta name="description" content="Nopunk glasses" />
      ...
      <main>
        ...
        <span style={{
          ...
          Add no punk glasses on any image
        </span>
        ...
        <div style={{ display: 'flex', gap: 20, flexDirection: 'column', width: '50%' }}>
          ...
          <input type="range" min={0.5} max={10} step={0.0001} value={scale} onChange={(e) => setScale(e.target.value)} />
          <label>
            Rotate
          </label>
        ...
        <button onClick={() => {
          ...
          a.href = dataURL
          a.download = `nopunkglasses-${Date.now()}.png`
          a.click()
        }} style={{
          marginTop: 20
        }}>
