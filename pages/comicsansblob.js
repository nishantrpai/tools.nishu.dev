...
export default function ComicSansBlob() {
  const [image, setImage] = useState(null)
  const [blobPosition, setBlobPosition] = useState({ x: 0, y: 0 })
  const [blobSize, setBlobSize] = useState(50)
  const [text, setText] = useState('comic sans')
  const [blobColor, setBlobColor] = useState('#FFE81F')
  ...
  const handleDownload = async () => {
    ...
    link.download = 'comic-sans-blob.png'
  ...
  return (
    <>
      <Head>
        <title>Comic Sans Blob</title>
        <meta name="description" content="Add Comic Sans blobs to images" />
        <link rel="icon" href="/favicon.ico" />
      </Head>
      <main>
        <h1>Comic Sans Blob</h1>
        <h2 className={styles.description}>Add Comic Sans blobs to your images</h2>
