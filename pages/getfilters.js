// get all filters applied to an image
...
export default function WhichFont() {
  ...
  const getFont = async () => {
    ...
    const response = await fetch('/api/gpt', {
      method: 'POST',
      body: JSON.stringify({
        prompt: 'please provide numbered list of filters applied to the image. only mention the filters.\n\nFilters:',
        image_url: dataURL,
        model: 'gpt-4o-mini'
      }),
    });
  ...
  return (
    <>
      <Head>
        ...
        <meta name="description" content="Get filters applied to an image"/>
        <link rel="icon" href="/favicon.ico" />
      </Head>
      {/* input to upload, canvas to render and prompt to detect the font */}
      <main>
        <h1 className={styles.title}>Get Filters</h1>
        <h2 className={styles.description}>
          Get filters applied to an image
        </h2>
        <canvas id="canvas"
          width={500}
          height={500}
